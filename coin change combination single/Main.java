public class Main {
    ...
    // ==================================================
    // 1) PURE RECURSION (PRINTS PATHS) — SINGLE USE
    // ==================================================
    static int recur(int[] arr, int idx, int tar, String path) {
        ...
        for (int i = idx; i < arr.length; i++) {
            if (tar - arr[i] >= 0) {
                ...
                count += recur(arr, i + 1, tar - arr[i], path + arr[i] + " ");
            }
        }
        return count;
    }
    ...
    // ==================================================
    // 2) 2D DP (MEMOIZATION) — SINGLE USE
    ...
    static int fillDP(int[] arr, int idx, int tar, Integer[][] dp) {
        ...
        for (int i = idx; i < arr.length; i++) {
            if (tar - arr[i] >= 0) {
                // i + 1  → single use
                count += fillDP(arr, i + 1, tar - arr[i], dp);
            }
        }
    ...
    // ==================================================
    // 3) BACKTRACK USING 2D DP (PRINT PATHS) — SINGLE USE
    // ==================================================
    static void printFromDP(int[] arr, int idx, int tar,
        ...
        for (int i = idx; i < arr.length; i++) {
            ...
            if (tar - ele >= 0 &&
                ...
                // i + 1  → single use
                printFromDP(arr, i + 1, tar - ele, dp, path + ele + " ");
            }
        }
    }

    // ==================================================
    // 4) 1D DP (COUNT ONLY – COMBINATIONS, SINGLE USE)
    // ==================================================
    static int coinChange1D(int[] coins, int target) {
        ...
        for (int coin : coins) {                 // fixes idx implicitly
            for (int t = target; t >= coin; t--) {
                // backward loop prevents reuse
                dp[t] += dp[t - coin];
            }
        }
